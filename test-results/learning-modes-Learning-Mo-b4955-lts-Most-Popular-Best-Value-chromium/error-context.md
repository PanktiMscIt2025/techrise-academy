...
- Name: learning-modes.spec.ts >> Learning Modes page >> badges shown: Best Results, Most Popular, Best Value
- Location: tests\learning-modes.spec.ts:20:7
...
Expected: visible
Error: strict mode violation: getByText('Most Popular') resolved to 2 elements:
    1) <span class="text-xs font-bold px-3 py-1 rounded-full text-white">Most Popular</span> aka getByText('Most Popular', { exact: true })
    2) <p class="text-sm text-gray-500 leading-relaxed mb-6">Our most popular mode. Small batches of 10–15 stu…</p> aka getByText('Our most popular mode. Small')
