...
- Name: homepage.spec.ts >> Homepage >> footer has brand name and contact email
- Location: tests\homepage.spec.ts:77:7
...
Locator: locator('footer').getByText(/TechRise/)
Expected: visible
Error: strict mode violation: locator('footer').getByText(/TechRise/) resolved to 3 elements:
    1) <span class="font-heading font-bold text-lg text-navy group-hover:text-saffron transition-colors">TechRise</span> aka getByRole('contentinfo').getByRole('link', { name: 'TechRise Academy' })
    2) <a href="/about" class="text-sm text-gray-400 hover:text-white transition-colors">About TechRise</a> aka getByRole('link', { name: 'About TechRise' })
    3) <p>…</p> aka getByText('© 2026 TechRise Academy. All')

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for locator('footer').getByText(/TechRise/)
